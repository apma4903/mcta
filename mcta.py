...
def _string2list(str, TYPE="int"):  # for strings of the format (#:1,2,3,4,...)
    ...
    if lst == [""]:
        ...
        raise ValueError(
            "error - unknown type passed to function _string2list: " + TYPE
        )
    ...
    for item in GeoJSON["features"]:
        ...
        if totalprob != 1:
            if round(totalprob, 5) == 1:
                if _verbose:
                    print(
                        "\t\tcorrecting ({:}) rounding error at link: {:}".format(
                            1 - totalprob, id
                        )
                    )
                corrections1 += 1
                ...
                if remainder == 0:
                    ...
                    if _verbose:
                        print(
                            "\t\tcorrecting ({:}) rounding error at link: {:} AGAIN! remainder added to P[i,i]".format(
                                remainder, id
                            )
                        )
                else:
                    assert (
                        False
                    ), "problem while correcting rounding errors at link {:}. TotalProb = {:}".format(
                        id, totalprob
                    )
            else:
                print(
                    "\t*** probabilities don't add up to 1 for link {:}, totalprob = {:}".format(
                        id, totalprob
                    )
                )
                FoundErrors += str(id) + ", "
    if _verbose and corrections1 + corrections2 > 0:
        print(
            "\tcorrected probability rounding errors in:\n\t\t{:} links from the 1st try\n\t\t{:} links from the 2nd try\n".format(
                corrections1, corrections2
            )
        )
    assert FoundErrors == "", (
        "error - please correct the above issues befor continuing for links: "
        + FoundErrors[:-2]
    )
    Psum = np.sum(P.todense(), axis=1)
    assert np.allclose(
        Psum, np.ones(Psum.shape[0])
    ), "error - The generated turning probabilities matrix 'P' is not a stochastic matrix"
    if _verbose:
        global _results
        _results["P_org"] = [
            str([", ".join("{:.3f}".format(i) for i in P.todense().tolist()[j])])
            .replace("'", "")
            .replace("[", "")
            .replace("]", "")
...
def _ModifyForLinkTravelTime(P):
    ...
    # OLD: ltt = np.array(
    ltt = np.array(
        [v["length"] / v["speed"] for k, v in sorted(_links.items())]
    )  # time = len / spd
    if _verbose:
        print("ltt=", ltt)
    ltt_min = min(ltt)
    ...
    ltt /= ltt_min
    if _verbose:
        print("ltt/ltt_min=", ltt)
    lttv = (ltt - 1) / ltt  # probability of staying; Pii
    if _verbose:
        print("lttv=", lttv)
    lttd = np.diag(lttv)  # lttv as a square matrix with a diagonal of its values
    if _verbose:
        print("lttd=", lttd)
    f = np.transpose(
        (1 - lttv)[np.newaxis]
    )  # use _sp.newaxis to create a 2D array that can be transposed
    if _verbose:
        print("f=", f)
    ...
    assert np.allclose(
        Psum, np.ones(Psum.shape[0])
    ), "error - the modified 'P' matrix, for link travel time, is not a stochastic matrix"
    if not np.allclose(P_org.todense(), P.todense()):
        if _verbose:
            print(
        ...
        print(
            "warning - link travel time did not modify P! check link lengths, speeds and results."
        )
    if _verbose:
        global _results
        _results["P_updated"] = [
...
def SetupMCTA(GeoJSON, Verbose=False):
    ...
    _verbose = Verbose
    _links = {
        int(k): v for k, v in GeoJSON["mcta_json"]["links"].items()
    }  # convert dictionary keys to int since JSON require them to be string
    _ReverseLinks = GeoJSON["mcta_json"]["LinkReverseDirection"]
    # OLD: _results["linkIDs"] = np.array(list(_links.keys())) + 1  # streets; start from 1
    # NEW:
    _results["linkIDs"] = np.array(list(_links.keys())) + 1  # streets; start from 1
    lengths = np.array([_links[id]["length"] for id in _links.keys()])
    lanes = np.array([_links[id]["lanes"] for id in _links.keys()])
    FreeFlowSpeeds = np.array([_links[id]["speed"] for id in _links.keys()])
...
def _FindEigen(P, Calc_C=True, SkipChecksForSpeed=False):
    ...
    if not SkipChecksForSpeed:
        assert (
            eigenvalues[0].imag == 0j
        ), "error - Perron Root value is complex ({:}+{:}j)".format(
            eigenvalues[0].real, eigenvalues[0].imag
        )
        assert (
            round(eigenvalues[0].real, 6) == 1
        ), "error - cannot find Perron Root (eigenvalue {:} != 1)".format(
            eigenvalues[0].real
        )
        for j in range(eigenvectors.shape[1]):
            assert (
                eigenvectors[j, 0].imag == 0j
            ), "error - Perron Root eigen vector include complex value @ {:}: {:}+{:}j".format(
                j, eigenvectors[0, j].real, eigenvectors[0, j].imag
            )

    PI = np.matrix(eigenvectors[:, 0].real).A1
    SUM = PI.sum()
    if (
        SUM != 1
    ):  # scaling an eigenvector by scalar value "a" does not change the eigenvector: P (a*v) = Lamda (a*v), where a*v is the eigen vector
        if _verbose and not SkipChecksForSpeed:
            print(
                "\tNormalizing eigen vector for Perron Root by a scaling factor of: {:}".format(
                    SUM
                )
            )
        PI /= PI.sum()
    ...
    if Calc_C:
        C = np.matrix(eigenvectors[:, 1].real).A1
...
def _GeneralInverse(P):
    ...
    Q = np.matrix(np.identity(P.shape[0]) - P)
    Qk = Q
    Qk1 = np.dot(Qk, Qk)
    ...
    Qi = Qk * np.dot(X, X)  # Ad =  A^(k) * X^(k+1)
    assert np.allclose(
        np.dot(Q, Qi), np.dot(Qi, Q)
    ), "general inverse 1st property invalid (Q)(Q#) = (Q#)(Q)"
    assert np.allclose(
        np.dot(np.dot(Q, Qi), Q), Q
    ), "general inverse 2nd property invalid (Q)(Q#)(Q) = (Q)"
    assert np.allclose(
        np.dot(np.dot(Qi, Q), Qi), Qi
    ), "general inverse 3rd property invalid (Q#)(Q)(Q#) = (Q#)"
    return Qi
    ...
    for i in range(P.shape[0]):
        for j in range(P.shape[1]):
            if i == j:
                ...
                Pi[j] == 0
            ):  # added to account for when closed states are not deleted from the matrix (when mcta_edit.Delete_States = False)
                m[i, j] = 0
            else:
...
def SolveMCTA(
    ...
    pi, c, eigenvalues = _FindEigen(
        P, "C" in Objectives, SkipChecksForSpeed
    )  # solve MC
    if "C" in Objectives:
        _results["Clusters"] = c
    if "PI" in Objectives:
    ...
    if "K" in Objectives:
        ...
        if any([x > 0.999999 for x in eigenvalues[1:]]):  # removed round call for speed
            ...
            ClosedStatesCount = P.shape[0] - np.count_nonzero(P.sum(axis=0))
            if (
                ClosedStatesCount > 0
            ):  # are there any closed but not deleted states? (when mcta_edit.Delete_States = False)
                K -= ClosedStatesCount
                if _verbose and not SkipChecksForSpeed:
                    assert P.sum(axis=0) == P.sum(
                        axis=1
                    ), "Error, number of closed states in P columns and rows do not match!"
            ResultMsg = "success"
        if not SkipChecksForSpeed:
            ...
            assert (
                round(K - K1, 6) == 0
            ), "Kemeny constant calculation methods do not agree! ( {:} != {:} )".format(
                K, K1
            )
    ...
    if "D" in Objectives:
        D = (
            VehiclesCount * pi / (lengths * lanes)
        )  # calculate vehicle density - vehicles/(km.lane)
        # if D.min()<0:
        # 	print(f'found non-ergodic network with negative Densities\nD_min = {D.min()}\K={K}')
        D = np.clip(
            D, 0, None
        )  # clip Density values to the interval (0,inf), as non-ergodic networks will produce
    ...
    if "E" in Objectives:
        assert len(FreeFlowSpeeds) == len(
            lanes
        ), f"Missing FreeFlowSpeeds for some or all lanes ({FreeFlowSpeeds})"
        Speeds, _ = _em.Speed_via_Density(
            D, "veh/(km.lane)", FreeFlowSpeeds, "km/h"
        )  # estimate vehicles speed on each link
        _results["Speeds"] = Speeds
        EC, _ = _em.ExternalCost(
            Speeds, "km/h", Method="TRANSYT7f"
        )  # Calculate average vehicle emissions cost $/(km.veh)
        _results["EmissionCost($/km)"] = (
            EC * VehiclesCount * pi
        )  # Calculate average link emissions $/km (in less operations)
        _results["EmissionCost($/hr)"] = (
            EC * lengths * D * Speeds
        )  # Calculate average link emissions $/hr (in less operations)
        _results["TotalNetworkEmissionCost"] = sum(
            EC * lengths * D * Speeds
        )  # Calculate total network emissions $/hr (Emissions * flow, while flow = Density * Speed)

    if _verbose and not SkipChecksForSpeed:
        print("\tMCTA Message: " + ResultMsg)
        if "K" in Objectives:
            print(
                "\tKemeny constant: {:.4f} ".format(K)
                + " steps ({:.4f} ".format(K * _results["StepTime"] * 60)
                + " min)"
            )
            print(
                "\texpecting time to mixing: {:.4f} ".format(K + 1)
                + " steps ({:.4f} ".format((K + 1) * _results["StepTime"] * 60)
                + " min)"
            )
        if "D" in Objectives:
        ...
        if "E" in Objectives:
            print(
                f"\tMin/Avg/Max network speeds (based on BPR): {min(Speeds):.2f}/{Speeds.mean():.2f}/{max(Speeds):.2f} km/h"
            )
            print(
...
if __name__ == "__main__":
    print(
        f"{_os.path.basename(__file__)} is designed for use as module, please see the doc string for usage."
    )
