...
_Density = _np.concatenate(
    [_np.arange(0, 100, 1), _np.arange(100, 250, 5)]
)  # density veh/(km.ln)
...
_Speed_min, _Speed_max = (
    1,
    120,
)  # parameters for setting up valid Speed range for root search
...
Costs = {"CO": 0.00052, "NOx": 0.00106, "VOC": 0.00140}
TimeAdjustedCosts = {
    k: v * 1.7898 for k, v in Costs.items()
}  # adjusted deom 1992 to 2018 using CPI change of 178.98%; https://www.officialdata.org/us/inflation/1992?amount=100
...
_BPR_coef = {
    120.0: {
        ...
        "b": 6.3,
        "type": "freeway",
    },  # C is based on Exhibts 21-2 & 23-2
    100.0: {
        "c": 2300,
        "a": 0.25,
        "b": 9.0,
        "type": "freeway",
    },  # minimal difference with 104km/h
    80.0: {"c": 2000, "a": 0.07, "b": 6.0, "type": "Multilane HW"},
    60.0: {"c": 1800, "a": 0.06, "b": 6.0, "type": "Multilane HW"},
}  # a and b estimated using regression line
...
def COPERT5(Speed, Speed_Units, Polutant):
    ...
    ) / (c["Epsilon"] * Speed**2 + c["Zeta"] * Speed + c["Eta"]) * (
        1 - c["RF"]
    ), "g/(km.veh)"
...
def TRANSYT7f(Speed, Speed_Units, Polutant):
    ...
    return (
        c["A"] * _np.exp(c["B"] * Speed_ft_per_sec) / (c["C"] * Speed_ft_per_sec)
    ) * _ft_per_km, "g/(km.veh)"
...
def ExternalCost(Speed, Speed_Units, Method="TRANSYT7f"):
    ...
    if Speed_Units != "km/h":
        raise ValueError(f"Speed must be provided in km/h units")
    return (
        sum([EF(Speed, Speed_Units, p)[0] * TimeAdjustedCosts[p] for p in Polutants]),
        "$/(km.veh)",
    )


def TravelTime_BPR_via_Flow(
    Flow,
    Flow_Units,
    FreeFlowSpeed,
    FreeFlowSpeed_Units,
...
def _Speed_BPR_via_Density(Speed, FreeFlowSpeed, Density, Method="old"):
    ...
    return (
        1 / Speed ** (b + 1)
        - (1 / FreeFlowSpeed) * (1 / Speed**b)
        - (a * (1 + a) ** b * (Density / Dc) ** b * 1 / FreeFlowSpeed ** (b + 1))
    )


_Speed_functions = (
    {}
)  # place holder for TravelTime(via Density) interpolation functions initialized later
...
def Speed_via_Density(Density, Density_Units, FreeFlowSpeed, FreeFlowSpeed_Units):
    ...
    return (
        _np.array(
            [
                _Speed_functions[ffs](D) if D < _MaxDensity else _SpeedAtMaxDensity
                for ffs, D in zip(FreeFlowSpeed, Density)
            ]
        ),
        "km/h",
    )
