...
def ModifyNetworkAndSolveMC(
    P, LinksEdits, VehiclesCount=1, Objectives=["D", "K", "E"], SkipChecksForSpeed=False
):
    ...
    if not SkipChecksForSpeed:
        assert (
            len(LinksEdits) == nl / 2
        ), "Size of LinksEdits must be half of the number of links in the network"
        assert all(
            x >= 0 and x <= 2 for x in LinksEdits
        ), "LinksEdits include value(s) out of bound (0,1,2)"
    ...
    lanes = _sp.array(
        [
            X[i, i] * links[i]["lanes"] + (1 - X[r[i], r[i]]) * links[r[i]]["lanes"]
            for i in links.keys()
        ]
    )
    ...
    if Delete_States:
        ...
        if not SkipChecksForSpeed:
            assert _sp.allclose(
                Q * ONES, ONES
            ), "not all rows in Q include a value of 1, not a proper permutation (orthogonal matrix), please check!\n{:}".format(
                Q
            )
            assert _sp.allclose(
                Q.T * ONES, ONES
            ), "not all columns in Q include a value of 1; not a proper permutation (orthogonal matrix), please check!\n{:}".format(
                Q
            )
            assert _sp.allclose(
                Q * Q.T, _sp.eye(nl)
            ), "Q*Q' is not an identity matrix; not a proper permutation (orthogonal matrix), please check!\n{:}".format(
                Q
            )
        ...
        if verbose:
            print(
                "\t** state closure method: Set closed states probabilities to zero in P matrix, and reorder P col/rows"
            )

    Results = _mcta.SolveMCTA(
        lengths=lengths,
        lanes=lanes,
        P=C,
    ...
    NoPostSolveEditsNeeded = [
        ...
        "KemenyConst",
        "Message",
    ]  # skip items not changed in pre-solve edits
    ...
    if verbose and Results["Message"] != "success":
        ...
        print(
            "Network has the following links\n\tGene: \t\t{:}\n\tClosed: \t{:}\n\tRemoved: \t{:}\n\tRemaining: \t{:}\n".format(
                LinksEdits,
                closed,
                [x for x in REMOVE if x not in closed],
                [x for x in range(len(links)) if x not in REMOVE],
            )
        )
