...
def _Plot2D(X, Y, Xlabel, Ylabel, Title, Limit=0, Yaxis_lim=[]):
    ...
    if Limit > 0:
        DotColor = [
            _CMAP(int(Y[i] / Limit * 254)) if Y[i] < Limit else _CMAP(254)
            for i in range(Y.shape[0])
        ]
    else:
        DotColor = [
            _CMAP(int((Y[i] - Y.min()) / (Y.max() - Y.min()) * 254))
            for i in range(Y.shape[0])
        ]
...
def _Plot3D(Z, Title):
    ...
    ax1.set_title(Title)
    X, Y = np.meshgrid(range(Z.shape[0]), range(Z.shape[0]))
...
def new_RenderMap(data, title, xlabel, limit):
    import matplotlib.pyplot as plt

    fig, ax = plt.subplots()

    # OLD cb = ax.imshow(data, aspect='auto')  # Create a mappable for the data
    # plt.title(title)
    # plt.xlabel(xlabel)
    # plt.colorbar(cb, ax=ax)  # Explicitly provide ax to determine the space for the colorbar
    # plt.xlim(limit)

    # NEW
    # Check the dimensionality of the data
    if data.ndim == 1:
        # If the data is 1D, plot as a line plot
        ax.plot(data)
        ax.set_ylabel("Value")
    elif data.ndim == 2:
        # If the data is 2D, plot as a heatmap
...
def _RenderMap(X, Title="RoadNetwork", LinkEndMark="s", Limit=0):
    ...
    if Limit > 0:
        LinksColor = [
            _CMAP(int(X[i] / Limit * 254)) if X[i] < Limit else _CMAP(254)
            for i in range(X.shape[0])
        ]
    else:
        LinksColor = [
            _CMAP(int((X[i] - X.min()) / (X.max() - X.min()) * 254))
            for i in range(X.shape[0])
        ]
    ...
    if Limit > 0:
        Z = [[0, 0], [0, 0]]
        levels = [x for x in np.arange(0, Limit * 1.1, Limit / 10)]
    else:
        step = (X.max() - X.min()) / 10
        Z = [[0, 0], [0, 0]]
        levels = [x for x in np.arange(X.min(), X.max() + step, step)]
...
def _Show_Save_Fig():
    ...
    if _figs2png:
        ...
        _plt.savefig(
            _BASE + "-Results-Fig" + str(_Show_Save_Fig.FigNum) + ".png",
            dpi=_Settings["DPI_SETTING"],
            bbox_inches="tight",
        )
        _Show_Save_Fig.FigNum += 1
    if _showfigs:
...
def Generate_Figure(Results, Variable, Limit=0):
    info = {
        "StationaryProb": {
            ...
            "title": "Stationary probability distribution of vehicles",
        },
        "Density": {
            "x": "Roads IDs",
            "y": "Density",
            "title": "Road traffic density $\\left(\\frac{vehicles}{km \\cdot lane}\\right)$",
        },
        "Clusters": {
            "x": "Roads IDs",
            "y": "Entries of the second eigenvector",
            "title": "Clusters in road network",
        },
        "Emission": {
            "x": "Roads IDs",
            "y": "Road's average CO emissions (g/km)",
            "title": "Average CO emissions (g/km)",
        },
        "EmissionCost($/hr)": {
            "x": "Roads IDs",
            "y": "Road Emissions External Cost ($/hr)",
            "title": "Road Emissions External Costs($/hr)",
        },
        "EmissionCost($/km)": {
            "x": "Roads IDs",
            "y": "Road Emissions External Cost ($/km)",
            "title": "Road Emissions External Costs($/km)",
        },
    }
...
def ShowModificationOnMap(
    EditedLinks,
):  # 14 and 27 are arbitrary density values for color coding only
