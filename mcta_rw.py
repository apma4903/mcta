...
class ComplexEncoder(_json.JSONEncoder):
    def default(self, obj):
        ...
        return _json.JSONEncoder.default(
            self, obj
...
def SaveResults(Results):
    ...
    for k, v in Results.items():
        if k in [
            ...
            MemDump[k] = [
                str([", ".join("{:05.0f}".format(i) for i in v.tolist()[j])])
                .replace("'", "")
                .replace("[", "")
                .replace("]", "")
            ...
            print(
                f"Unrecognized result {k}. it will not be included in the saved results."
            )
