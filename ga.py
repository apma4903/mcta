...
CXPB, MUTPB, IndpProb = (
    ...
    0.18,
    0.01,
)  # crossover prob, mutation prob, individual gene mutations prob
nObjectives = 2  # multi-objective optimization?
MCTA_BaseFile = "AD2"

nFitnessValues = None
ga_data = {}
ga_data["GA_Parameters"] = {
    "MCTA_BaseFile": MCTA_BaseFile,
    "AllGenes": AllGenes,
    "chromosome_size": chromosome_size,
    "population_size": population_size,
    "KeepBetweenGens": KeepBetweenGens,
...
def evalObj(individual):
    R = mcta_edit.ModifyNetworkAndSolveMC(
        P, individual, GeoJSON["mcta_json"]["VehiclesCountEst"], SkipChecksForSpeed=True
    )
    # with numpy.errstate(all='ignore'):
    return (
        R["Density"].max(),
        # R['KemenyConst'].real,
        R["TotalNetworkEmissionCost"],
...
toolbox.register(
    "mutate", mutFlipNum, indpb=IndpProb
...
if nObjectives > 1:
    ...
    )  # operator for selecting individuals for breeding the next generation: non dominant sorting using NSGA2 algorithm
    HoF = tools.ParetoFront()
else:
    toolbox.register(
        "select", tools.selBest
    )  # operator for selecting best K individuals for breeding the next generation
...
def main():
    ...
    for j in range(round(population_size * 0.05)):
        for i in range(
            chromosome_size
        ):  # set 5% of individuals to current road network state (all genes 0)
            pop[j][i] = 0
    ...
    print2(
        f"Optimizing using {nObjectives} objective(s)\nTracking {nFitnessValues} fitness value(s)\n"
    )
    ...
    print2(
        "  Generation {: 4d} - Evaluated {: 4d} individuals. {:}. time:{: 8.3f} min.".format(
            0, len(pop), ObjStats(Cur_best, Old_best), (time.time() - tic) / 60
        )
    )
    # with numpy.errstate(all='ignore'):
    ...
    for g in range(num_generations):  # Begin the evolution
        ...
        tmp = toolbox.select(
            offspring, KeepBetweenGens
        )  # Select the next generation individuals
        pop = list(toolbox.map(toolbox.clone, tmp)) * (
            len(pop) // KeepBetweenGens
        )  # Clone the selected individuals
...
def ga_batchrun(
    MaxTime=600, nGn=999999, Psz=200, Ktp=0.1, Cpr=0.5, Mpr=0.30, gMpr=0.01
):
